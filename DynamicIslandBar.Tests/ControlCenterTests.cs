...
public class ControlCenterTests
{
    ...
    public void ControlCenter_XamlContainsFourPagesAndFeedbackAttachmentSurface()
    {
        ...
        Assert.Contains("x:Key=\"PrimaryTextBrush\"", xaml);
        Assert.Contains("x:Key=\"InactiveTextBrush\"", xaml);
        Assert.Contains("Foreground=\"{DynamicResource PrimaryTextBrush}\"", xaml);
        Assert.Contains("x:Key=\"FeatureSearchResultItemStyle\"", xaml);
        Assert.DoesNotContain("Value=\"#2834C759\"", xaml);
        Assert.DoesNotContain("Value=\"#7034C759\"", xaml);
        Assert.Contains("Property=\"BorderBrush\" Value=\"{StaticResource SelectedGlassBorderBrush}\"", xaml);
        Assert.Contains("FeatureSearchSubmitButton_Click", code);
        Assert.Contains("<Setter Property=\"Foreground\" Value=\"{StaticResource AccentBrush}\" />", xaml);
        Assert.Contains("x:Key=\"FeatureSearchSurfaceBrushColor\">#00000000", xaml);
        Assert.Contains("Color=\"{DynamicResource PrimaryTextBrushColor}\"", xaml);
        ...
        Assert.Contains("ApplyThemeGradientColors", code);
        Assert.Contains("SetBrushColor(\"PrimaryTextBrush\", colors.PrimaryText);", code);
        Assert.Contains("Color.FromArgb(0xD8, 0xFF, 0xFF, 0xFF)", code);
        Assert.Contains("Resources[colorResourceKey] = color;", code);
